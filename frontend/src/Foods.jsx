    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input className="input" type="number" min="1" value={form.original_ml_g} onChange={set('original_ml_g')} required />
        </div>
      </div>
    ...
    <tr className="border-b border-gray-100 hover:bg-cream transition-colors">
      ...
      <td className="py-3 px-4">
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => onEdit(food)} className="btn-secondary text-xs px-3 py-1.5">Edit</button>
    ...
    <div className="p-8 max-w-6xl">
        ...
        <div className="card p-0 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-cream-dark border-b border-gray-200">
              <tr>
                {['Name', 'Best before', 'Status', 'Remaining / Original', 'Actions'].map(h => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-10 text-center font-body text-sm text-ink-muted">
                    No items found.
                  </td>
                </tr>
              ) : (
      ...
      {modal === 'edit' && selected && (
        <Modal title="Edit food item" onClose={() => setModal(null)}>
          <FoodForm
            initial={{
              name: selected.name, remarks: selected.remarks ?? '',
              bestBeforeEnd: selected.bestBeforeEnd, original_ml_g: selected.original_ml_g,
            }}
            onSave={handleEdit}
